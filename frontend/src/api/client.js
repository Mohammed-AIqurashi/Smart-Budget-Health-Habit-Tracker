...
const API_URL = import.meta.env.VITE_API_URL ||
  (typeof window !== 'undefined' && window.location.hostname.includes('onrender.com')
    ? 'https://smart-budget-backend-aokn.onrender.com/api'
    : '/api');
...
// ── Backend Warmup Ping ────────────────────────────────────────────────────────
// Render free tier cold-starts can take 30-60 s. Fire a silent health ping as
// soon as this module is imported so the backend wakes up before the user's
// first real request. Runs only in production (onrender.com or custom domain).
if (typeof window !== 'undefined' && !window.location.hostname.includes('localhost')) {
  fetch(`${API_URL}/health`, { method: 'GET' }).catch(() => {/* silent – warmup only */});
}

// ─── Demo Store Helpers ───────────────────────────────────────────────────────
const getUsersList = () => {
  const list = JSON.parse(localStorage.getItem('demo_users')) || [];
  return list;
};
...
const buildDashboard = () => {
  ...
  const effectiveBudget = (Number(user.monthlyBudget) || 0) + totalIncome;
  ...
  return {
    summary: {
      remainingBudget: Math.max(0, effectiveBudget - totalExpenses),
      totalExpenses,
      budgetPercentage: effectiveBudget > 0
        ? Math.min(100, Math.round((totalExpenses / effectiveBudget) * 100)) : 0,
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (url.includes('/habits')) {
      ...
      const summary = filtered.reduce(
        (acc, h) => {
          if (h.metricName === 'calories') {
            acc.calories += Number(h.value) || 0;
            acc.protein += Number(h.protein) || 0;
            acc.carbs += Number(h.carbs) || 0;
            acc.fat += Number(h.fat) || 0;
          } else if (h.metricName === 'water') {
            acc.water += Number(h.value) || 0;
          } else if (h.metricName === 'steps') {
            acc.steps += Number(h.value) || 0;
          } else if (h.metricName === 'sleep') {
            acc.sleep += Number(h.value) || 0;
          }
          return acc;
        },
      ...
      return { data: { habitLogs: paginated, habits: paginated, summary, totalPages, currentPage: page, pagination: { page, limit, total, totalPages } } };
    }
